#!/usr/bin/env python3
"""
setup_gmail_solar_filter.py — Create Gmail label "Suppliers/Solar" and a filter
that applies it to incoming mail from the solar slewing drive supplier shortlist
in data/china_solar_slewing_drive_suppliers.json.
...
Usage:
    python scripts/setup_gmail_solar_filter.py
    python scripts/setup_gmail_solar_filter.py --dry-run
"""
...
def ensure_label(service, name: str, dry_run: bool) -> str | None:
    ...
    name "Suppliers/Solar" produces the nested structure.
    """
...
def main():
    ...
    logger.info("=" * 60)
    logger.info("  GMAIL LABEL + FILTER SETUP — SUPPLIERS/SOLAR")
    logger.info("=" * 60)
