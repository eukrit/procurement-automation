#!/usr/bin/env python3
"""
setup_gmail_rice_export_filter.py — Create Gmail label "RFQ/2026 Rice Export to China"
and a filter that applies it to incoming mail from the Thai rice exporter
shortlist in data/thailand_rice_exporters.json.
...
Usage:
    python scripts/setup_gmail_rice_export_filter.py
    python scripts/setup_gmail_rice_export_filter.py --dry-run
"""
...
LABEL_NAME = "RFQ/2026 Rice Export to China"
...
def ensure_label(service, name: str, dry_run: bool) -> str | None:
    ...
    name "RFQ/2026 Rice Export to China" produces the nested structure.
    """
...
def main():
    ...
    logger.info("=" * 60)
    logger.info("  GMAIL LABEL + FILTER SETUP — RICE EXPORT RFQ")
    logger.info("=" * 60)
    ...
    try:
        ...
        logger.error("Service account key not found: %s", exc)
        sys.exit(1)
