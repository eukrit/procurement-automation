#!/usr/bin/env python3
"""
setup_gmail_solar_pv_filter.py — Extend the existing Gmail label
"Suppliers/Solar" with a filter that auto-applies it to inbound mail from
the solar PV (module / panel) supplier shortlist in
data/china_solar_pv_suppliers.json.

Coexists with `setup_gmail_solar_filter.py`, which handles the solar
slewing-drive supplier list for the same label. Each script manages its
own `from:` filter against the shared `Suppliers/Solar` label.
...
Usage:
    python scripts/setup_gmail_solar_pv_filter.py
    python scripts/setup_gmail_solar_pv_filter.py --dry-run
"""
...
def main():
    ...
    logger.info("=" * 60)
    logger.info("  GMAIL LABEL + FILTER SETUP — Suppliers/Solar (PV modules)")
    logger.info("=" * 60)
